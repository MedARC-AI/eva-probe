...
def load_mask(fullmask):
    try:
        with Image.open(fullmask) as mask:
            return np.array(mask)
    except Exception as e:
        print(f"\nError loading mask: {fullmask}")
        print(f"Error details: {str(e)}")
        print("This mask file is corrupted. It will be skipped.")
        return None
...
def open_jpg(img):
    try:
        ...
        print(f"\nError loading image: {img}")
        print(f"Error details: {str(e)}")
        print("This file appears to be corrupted. It will be skipped.")
        return None
...
class ImageProcessor():
    def __init__(self, path_images, path_masks, path_patches):
        ...
        os.makedirs(self.path_masks, exist_ok=True)
        os.makedirs(self.path_patches, exist_ok=True)
        ...
        for filename in os.listdir(self.path_images):
            if filename.startswith(tma_pref):
                ...
                if os.path.exists(fullmask):
                    ...
                    if img is None:
                        continue  # Skip to next file if image couldn't be loaded
                    ...
                    try:
                        ...
                        print(f"\nError loading mask: {fullmask}")
                        print(f"Error details: {str(e)}")
                        print("This mask file appears to be corrupted. It will be skipped.")
                        continue
                    if mask is None:
                        continue  # Skip to next file if mask couldn't be loaded
    ...
    def create_joint_patches(self, tma_pref, patch_size, csv_file, n_class=4):
        ...
        os.makedirs(subdir_patho_1, exist_ok=True)
        os.makedirs(subdir_patho_2, exist_ok=True)
...
def main():
    ...
    # Accept command line arguments for input and output directories
    if len(sys.argv) >= 3:
    ...
    tma_info_path = os.path.join(output_prefix, 'tma_info')
    ...
    path_test_patches = os.path.join(output_prefix, 'test_patches_%d' % patch_size)
    ...
    print(f"Training/validation patches: {path_patches}")
    print(f"Test patches: {path_test_patches}")
    print(f"Summary files: {tma_info_path}")
